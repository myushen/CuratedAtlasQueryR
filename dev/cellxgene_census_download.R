...
get_census_data <- function(df, 
  ...
  results <- pmap(df, function(...) {
    ...
    obs_value_filter <- sprintf(
      "donor_id == '%s' & assay == '%s' & disease == '%s' & sex == '%s' & self_reported_ethnicity == '%s' & tissue == '%s' & development_stage == '%s'",
      args$donor_id,
      # escape single quote in some assays
      gsub("'", "\\\\'", args$assay),
      args$disease, 
      args$sex, 
      args$self_reported_ethnicity, 
      args$tissue, 
      args$development_stage
    )
    print(paste("Processing:", args$filename))
    if (experiment == "sce") {
      data <-
